...
from login.models import Team,Admin,Class,Student,Faculty,Calender,Course,Attendance,Timetable,Teache
...
def stud_report(request):
    ...
    for i in atte:
        if i.presence:
            writer.writerow([i.fac_id.fac_id,stud.get().class_id.class_id,stud.get().team_id.team_id,cou,i.date,'Present'])
        else:
            writer.writerow([i.fac_id.fac_id,stud.get().class_id.class_id,stud.get().team_id.team_id,cou,i.date,'Absent'])
    return response
